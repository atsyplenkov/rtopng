test_that("variogram model construction and updates cover default and mutation branches", {
  ...
  expect_s3_class(vm_default, "rtopVariogramModel")
  expect_identical(vm_default$model, "Ex1")
  expect_length(vm_default$params, 5)

  vm_from_obs <- rtopVariogramModel(
    observations = base_object$observations,
    formulaString = base_object$formulaString
  )
  expect_s3_class(vm_from_obs, "rtopVariogramModel")
  expect_length(vm_from_obs$params, 5)

  vm_mult <- updateRtopVariogram(vm_default, sill = 2, range = 3, nugget = 4, exp = 5, exp0 = 6, action = "mult")
  vm_replace <- updateRtopVariogram(vm_default, sill = 99, action = "replace")
  vm_add <- updateRtopVariogram(vm_default, exp = 1.5, action = "add")

  expect_equal(vm_mult$params[1], vm_default$params[1] * 2)
  expect_equal(vm_mult$params[2], vm_default$params[2] * 3)
  expect_equal(vm_replace$params[1], 99)
  expect_equal(vm_add$params[4], vm_default$params[4] * 1.5)
})

test_that("variogram fitting, discretization, and matrices cover spatial and sf paths", {
  spatial <- rtopng_spatial_subset_fixtures(n_obs = 12, n_pred = 2)
  sf_fixtures <- rtopng_sf_subset_fixtures(n_obs = 12, n_pred = 5)
  ...
  params_sf_cloud <- modifyList(sf_fixtures$params, list(cloud = TRUE, nugget = FALSE, model = "Ex1"))
  params_sf_cloud_false <- modifyList(sf_fixtures$params, list(cloud = FALSE, nugget = FALSE, model = "Ex1"))
...
test_that("overlap and wrapper helpers cover their direct branches", {
  ...
  est <- rtopng:::estimateParameters.rtop(
    createRtopObject(
      ...
      spatial$prediction_locations,
      formulaString = "obs ~ 1",
      params = modifyList(spatial$params, list(nugget = FALSE))
    )
  )
