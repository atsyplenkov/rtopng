...
rtopSim.rtop <- function(
  ...
  if (is.null(variogramModel)) {
    stop("Cannot do simulations without a variogram model")
  }
  if (
    length(object$observations) > 0 &&
      (is.null(object$varMatObs) || varMatUpdate)
  ) {
    object <- varMat(object, varMatUpdate, debug.level = debug.level, ...)
  }
  if (
    is.null(object$varMatPred) ||
      diff(dim(object$varMatPred)) != 0 ||
      varMatUpdate
  ) {
    if (
      is.null(object$dPred) && !(params$gDistPred && !is.null(object$gDistPred))
    ) {
    ...
    if (params$gDistPred && is.null(object$gDistPred)) {
      ...
      varMatPred <- object$varMatPred <- varMat(
        object$gDistPred,
        params = params,
  ...
  if (replace && !("replaceNumber" %in% names(predictionLocations))) {
    stop(
      "Cannot replace observations if predictionLocations does not have column with replaceNumber"
    )
  } else if (
    replace &&
      !(dim(observations)[1] >=
        max(predictionLocations$replaceNumber, na.rm = TRUE) &&
        min(predictionLocations$replaceNumber, na.rm = TRUE) >= 1)
  ) {
    stop(
      "predictionLocations$replaceNumber does not correspond with the number of observations"
    )
  }
  if (!is.null(dump)) {
  ...
  for (isim in 1:params$nsim) {
    ...
    for (ip in seq_along(ips)) {
      ...
      if (nobs == 0) {
        ...
        if (!is(ret, "try-error")) {
          ...
          if (
            replace &&
              !is.na(predictionLocations$replaceNumber)[inew] &&
              predictionLocations$replaceNumber[inew] != 0
          ) {
        ...
        if (
          ...
          nd <- dim(vObs)[1]
          if (varClean && any(vObs[nd, 1:(nd - 1)] < 1e-9)) {
    ...
    if (replace) {
      ...
      predictions$sim[ips] <- obs[(nobs0 + 1):length(obs)]
    }
...
rtopSim.default <- function(
  ...
  if (!missing(varMatPredObs)) {
    object$varMatPredObs <- varMatPredObs
  }
  if (!missing(varMatPred)) {
